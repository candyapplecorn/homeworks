...
#    # Your code goes here...
#		if %i{bone kibble treat}.include?(name) && args.empty?
#			description = capitalize @snack_box.send("get_#{name}_info".to_sym, @box_id)
#			tastiness = @snack_box.send("get_#{name}_tastiness".to_sym, @box_id)
#			return "#{capitalize(name)}: #{description}: #{tastiness}"
#		else
#			super
#		end
#  end
  ...
  def self.define_snack(name)
	...
	private
	def capitalize(s); s[0].upcase + s[1..-1].downcase; end
end
